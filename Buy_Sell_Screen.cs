...
public class Buy_Sell_Screen : MainScreen
{
	...
	public static void setmItemOther(MainItem item, sbyte type, sbyte index)
	{
		...
		case 3:
		{
			...
			{
				if (mItemOther[index][k] != null && mItemOther[index][k].ItemCatagory == item.ItemCatagory && mItemOther[index][k].Id == item.Id)
				{
					mItemOther[index][k] = null;
					if (index == 0)
					{
						mRedSell[k] = -1;
					}
		...
		case 2:
		{
			...
			{
				if (mItemOther[index][j] == null)
				{
					mItemOther[index][j] = item;
					break;
				}
			}
			break;
