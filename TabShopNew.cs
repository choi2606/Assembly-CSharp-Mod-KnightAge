...
public class TabShopNew : MainTabNew
{
	...
	public override void commandPointer(int index, object obj)
	{
		...
		if (index == 43)
		{
			...
			if (mainItem != null)
			{
				...
				item_sell o = new item_sell((short)mainItem.Id, price, num, mainItem.ItemCatagory);
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 0:
			...
			break;
		case 1:
			if ((Item)mVector3.elementAt(idSelect) != null)
			{
		...
		case 2:
		{
			MainItem mainItem11 = (MainItem)mVector3.elementAt(idSelect);
			if (mainItem11 != null)
			{
				int num9 = 1;
				try
				{
					num9 = int.Parse(inputDialog.tfInput.getText());
				}
				catch (Exception)
				{
					num9 = 1;
				}
				GlobalService.gI().buy_item((sbyte)IDTab, (short)mainItem11.Id, (short)num9);
				GameCanvas.end_Dialog();
			}
			break;
		}
		...
		case 6:
		{
			...
			if (mainItem8.ItemCatagory == 5)
			{
				GameCanvas.start_Ok_Dialog(T.khongthedung);
				break;
			}
			if (mainItem8.ItemCatagory == 4)
			{
				if (mainItem8.typePotion > 1 || Player.timeDelayPotion[mainItem8.typePotion].value <= 0)
				{
					if (GameScreen.help.setStep_Next(2, 4) && mainItem8.typePotion == 0)
					{
						GameScreen.help.Next++;
						GameScreen.help.setNext();
					}
					if (mainItem8.typePotion >= 2 || ((mainItem8.typePotion != 0 || GameScreen.player.hp != GameScreen.player.maxHp) && (mainItem8.typePotion != 1 || GameScreen.player.mp != GameScreen.player.maxMp)))
					{
						if (mainItem8.typePotion == Item.ID_TEM_VE_MUA_BAN)
						{
						...
						{
							GlobalService.gI().Use_Potion((short)mainItem8.Id);
							if (mainItem8.typePotion < 2)
							{
								Player.timeDelayPotion[mainItem8.typePotion].value = 2000;
								Player.timeDelayPotion[mainItem8.typePotion].limit = 2000;
								Player.timeDelayPotion[mainItem8.typePotion].timebegin = mSystem.currentTimeMillis();
							}
						}
					}
				}
			...
			{
				int num4 = 0;
				if (mainItem8.type_Only_Item < 12)
				{
					num4 = MainTemplateItem.mItem_Rotate_Tem_Equip[mainItem8.type_Only_Item];
				}
				if (num4 == -1)
				{
					mVector mVector8 = new mVector("TabShopNew menu");
					...
					mVector8.addElement(iCommand2);
					mVector8.addElement(iCommand3);
					GameCanvas.menu2.startAt(mVector8, 2, T.Ring, isFocus: false, null);
				}
				else
				{
					if (mainItem8.ItemCatagory == 7)
					{
						GlobalService.gI().doUseMaterial((short)mainItem8.Id);
					}
					else
					{
						GlobalService.gI().Use_Item((sbyte)mainItem8.Id, (sbyte)num4);
					}
		...
		case 7:
			...
			if (item != null)
			{
				mVector mVector4 = new mVector("TabShopNew menu2");
				mVector4 = doMenu(item);
				string text = T.item;
				if (isTypeShop == SHOP_BANG)
				{
					text = T.iconclan;
				}
				GameCanvas.menu2.startAt(mVector4, 2, text, isFocus: false, null);
		...
		case 8:
		{
			mVector mVector6 = new mVector("TabShopNew menu3");
			for (int i = 0; i < 5; i++)
			{
				...
				mVector6.addElement(o);
			}
			GameCanvas.menu2.startAt(mVector6, 2, T.setKey, isFocus: false, null);
		...
		case 14:
			...
			if (item != null)
			{
				...
				GlobalService.gI().Update_Char_Chest(SET_CHEST, (short)item.Id, (sbyte)item.ItemCatagory, (short)num7);
				if (GameCanvas.isTouch)
				{
					idSelect = -1;
				}
		...
		case 17:
		{
			mVector mVector5 = new mVector("TabShopNew menusell");
			mVector5.addElement(new iCommand(T.banhettrang, 19, 0, this));
			mVector5.addElement(new iCommand(T.banhetxanh, 19, 1, this));
			GameCanvas.menu2.startAt(mVector5, 2, T.sell, isFocus: false, null);
			break;
		}
		case 18:
		{
		...
		case 19:
		{
			...
			{
				MainItem mainItem10 = (MainItem)Item.VecInvetoryPlayer.elementAt(j);
				switch (subIndex)
				{
				case 0:
					if (mainItem10.ItemCatagory == 3 && mainItem10.colorNameItem == 0)
					{
						num8++;
					}
					break;
				case 1:
					if (mainItem10.ItemCatagory == 3 && mainItem10.colorNameItem == 1)
					{
						num8++;
					}
					break;
			...
			else
			{
				...
				{
				case 0:
					GameCanvas.start_Left_Dialog(T.hoisell + num8 + T.dotrang, new iCommand(T.sell, 18, 0, this));
					break;
				case 1:
					GameCanvas.start_Left_Dialog(T.hoisell + num8 + T.doxanh, new iCommand(T.sell, 18, 1, this));
					break;
				}
			}
			break;
		...
		case 29:
		{
			...
			mVector mVector11 = new mVector("TabShopNew vectab");
			...
			mVector11.addElement(tabShopNew);
			GameCanvas.foodPet = new TabScreenNew();
			GameCanvas.foodPet.selectTab = 0;
			GameCanvas.foodPet.addMoreTab(mVector11);
		...
		case 42:
		{
			...
			mVector10.addElement(new iCommand(T.yes, 43, item_sell2, this));
			mVector10.addElement(new iCommand(T.cancel, 35, this));
			GameCanvas.start_Select_Dialog(str3, mVector10);
			break;
		}
		case 44:
			inputDialog = new InputDialog();
		...
		case 46:
		{
			MainItem mainItem7 = (MainItem)mVector3.elementAt(idSelect);
			if (mainItem7 != null)
			{
				if (mainItem7.ItemCatagory == 3)
				{
					GlobalService.gI().do_Buy_Sell_Item(2, null, string.Empty, idSeller, mainItem7.Id, (sbyte)mainItem7.ItemCatagory);
				}
				else if (mainItem7.ItemCatagory == 7)
				{
					GlobalService.gI().do_Buy_Sell_Item(2, null, string.Empty, idSeller, mainItem7.Id, (sbyte)mainItem7.ItemCatagory);
				}
				else if (mainItem7.ItemCatagory == 4)
				{
					GlobalService.gI().do_Buy_Sell_Item(2, null, string.Empty, idSeller, mainItem7.Id, (sbyte)mainItem7.ItemCatagory);
				}
			}
		...
		case 52:
		{
			...
			if (mainItem6 != null)
			{
				...
				GlobalService.gI().Hop_rac(0, (short)mainItem6.Id, (sbyte)mainItem6.ItemCatagory);
			}
			vecListCmd.removeAllElements();
			break;
		}
		case 53:
		{
			MainItem mainItem5 = (MainItem)Item.VecInvetoryPlayer.elementAt(idSelect);
			if (mainItem5 != null)
			{
				GlobalService.gI().Hop_rac(1, (short)mainItem5.Id, (sbyte)mainItem5.ItemCatagory);
			}
			vecListCmd.removeAllElements();
			break;
		}
	...
	public mVector doMenu(Item item)
	{
		...
		if (typeTab == MainTabNew.INVEN_AND_STORE)
		{
			if (Item.VecItemSell.size() > 0)
			{
				...
				{
					if (((MainItem)Item.VecItemSell.elementAt(i)).Id == item.Id)
					{
						return mVector3;
					}
				}
		...
		else if (typeTab != MainTabNew.SELLITEM)
		{
			...
			else
			{
				...
				{
					item = (Item)vecShop.elementAt(idSelect);
					if (item != null || !item.setNameNull())
					{
						if (item.ItemCatagory == 4 || item.ItemCatagory == 7)
						{
	...
	public override void paint(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (typeTab == MainTabNew.INVEN_AND_STORE && !GameScreen.player.isSelling() && Item.VecItemSell.size() > 0)
			{
				...
				{
					if (((MainItem)Item.VecItemSell.elementAt(j)).Id == item.Id)
					{
			...
			if (isTypeShop == SHOP_NANG_CAP_MEDEL || isTypeShop == SHOP_ANY_NGUYEN_LIEU || isTypeShop == SHOP_KHAM_NGOC || isTypeShop == SHOP_GHEP_NGOC || isTypeShop == SHOP_DUC_LO)
			{
				...
				if (TabRebuildItem.vecGem.size() > 0)
				{
					...
					{
						MainItem mainItem = (MainItem)TabRebuildItem.vecGem.elementAt(k);
						g.setColor(268038149);
						if (mainItem != null && (item.typeMaterial == Item.TYPE_NGOC_KHAM || item.typeMaterial == Item.TYPE_BUA_HUYEN_BI || isTypeShop == SHOP_ANY_NGUYEN_LIEU) && item.Id == mainItem.Id && item.ItemCatagory == mainItem.ItemCatagory)
						{
							g.drawRect(xBegin + MainTabNew.wOneItem / 2 + i % numW * MainTabNew.wOneItem - MainTabNew.wOneItem / 2 + 4, yBegin + MainTabNew.wOneItem / 2 + i / numW * MainTabNew.wOneItem - MainTabNew.wOneItem / 2 + 4, MainTabNew.wOneItem - 8, MainTabNew.wOneItem - 8, mGraphics.isTrue);
						}
					}
