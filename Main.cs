...
public class Main : MonoBehaviour
{
	...
	private void Start()
	{
		if (!started)
		{
			...
			int width = int.Parse(array[3]);
			int height = int.Parse(array[4]);
	...
	{
		string directoryName = Path.GetDirectoryName(AutoManager.filePath);
		string fileName = Path.GetFileName(AutoManager.filePath);
		fileWatcher = new FileSystemWatcher(directoryName, fileName);
		fileWatcher.NotifyFilter = NotifyFilters.FileName | NotifyFilters.LastWrite;
		fileWatcher.Changed += OnFileChanged;
		fileWatcher.Created += OnFileChanged;
		fileWatcher.Deleted += OnFileChanged;
		fileWatcher.EnableRaisingEvents = true;
		Debug.Log("File watcher initialized and started.");
	}
	...
	private void OnFileChanged(object sender, FileSystemEventArgs e)
	{
		lock (mainThreadActions)
		{
			mainThreadActions.Enqueue(delegate
			{
				...
				if (File.Exists(AutoManager.filePath))
				{
					string text = File.ReadAllText(AutoManager.filePath);
	...
	{
		if (CRes.loadRMS("versionGame") != null)
		{
			try
			{
