...
public class CRes
{
	...
	private static void recQuickSort(mVector actors, int left, int right)
	{
		try
		{
			if (right - left > 0)
			{
				int ySort = ((MainObject)actors.elementAt(right)).ySort;
	...
	private static void recQuickSort1(mVector actors, int left, int right)
	{
		try
		{
			if (right - left > 0)
			{
				int y = ((MainItemMap)actors.elementAt(right)).y;
	...
	public static void getFromRms(sbyte type, short id)
	{
		...
		try
		{
			...
			data = new sbyte[dataInputStream.readInt()];
			dataInputStream.read(ref data);
			data2 = new sbyte[dataInputStream.readShort()];
