...
public class Player : MainObject
{
	...
	{
        AutoManager.autoFire(AutoManager.nameMon);
        AutoManager.khoaViTri();
        AutoManager.autoPB();
        AutoManager.hutMonster();
		AutoManager.autoHutPlayers();
        AutoManager.autoDoSat();
        /*if (AutoManager.config.IsAuto)
		{
            AutoManager.khoaViTri();
            AutoManager.fireMonstersByName(AutoManager.nameMon);
            AutoManager.autoPB();
            AutoManager.hutMonster();
        }
        if (AutoManager.config.IsSendChat)
        {
            AutoManager.sendChat();
            AutoManager.config.IsSendChat = false;
        }*/
        try
        {
	...
	{
		if (obj.typeObject != 4 && obj.typeObject != 3 && obj.typeObject != 5)
		{
			return obj.typeObject == 7;
		}
		return true;
	}
	...
	public void updateKey()
	{
		...
		else
		{
			if (Action != 4 && !isTanHinh)
			{
				...
				if (GameCanvas.keyMyHold[35] && Main.isPC)
				{
		...
		else if (GameScreen.ObjFocus.typeObject == 0)
		{
			if (setFirePlayer(GameScreen.ObjFocus))
			{
				if (mhotkey[levelTab][IndexFire].type != HotKey.SKILL)
				{
					...
					{
						if (mhotkey[levelTab][i].type == HotKey.SKILL)
						{
							IndexFire = (sbyte)i;
							if (isAutoFire == 1)
							{
