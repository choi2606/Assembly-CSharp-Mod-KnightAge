public class MainParty
{
	...
	public void setIsParty()
	{
		for (int i = 0; i < vecPartys.size(); i++)
		{
			((ObjectParty)vecPartys.elementAt(i)).ischeck = false;
		}
		for (int j = 0; j < GameScreen.Vecplayers.size(); j++)
		{
			...
			{
				ObjectParty objectParty = (ObjectParty)vecPartys.elementAt(k);
	...
	public void setObjParty(MainObject obj)
	{
		for (int i = 0; i < vecPartys.size(); i++)
		{
			if (((ObjectParty)vecPartys.elementAt(i)).name.CompareTo(obj.name) == 0)
			{
				obj.isParty = true;
				break;
			}
