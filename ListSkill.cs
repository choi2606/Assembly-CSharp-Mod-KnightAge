public class ListSkill : MainListSkill
{
	...
	{
		if (MainObject.getDistance(obj.x, obj.y, objBeKill.x, objBeKill.y - objBeKill.hOne / 4) <= range || obj.typeObject == 9)
		{
	...
	{
		return mSystem.currentTimeMillis() - lastTimeAttack > limitTimeAtt;
	}
	...
	public bool setBuff(int type)
	{
		...
		{
			if (((MainBuff)obj.vecBuff.elementAt(i)).typeBuff == type)
			{
				return false;
			}
		}
