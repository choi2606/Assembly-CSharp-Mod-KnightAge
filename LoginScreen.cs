...
public class LoginScreen : MainScreen
{
	...
	public static void loadUser_Pass()
	{
		...
		if (array != null)
		{
			new DataInputStream(array);
			string text = AutoManager.array5[0];
			string text2 = AutoManager.array5[1];
			int num = int.Parse(AutoManager.array5[2]);
			string host = GameCanvas.listServer[num, 1];
			int port = GameCanvas.portServer[num];
			Session_ME.gI().connect(host, port);
			tfusername.setText(text);
			tfpassword.setText(text2);
			string text3 = tfusername.getText();
			if (text3.Length >= 10)
			{
				text3 = text3.Substring(0, 10);
			}
			if (text3.CompareTo("knightauto") == 0)
			{
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		{
			_ = string.Empty;
			string link = (IndoServer.isIndoSv ? "http://ksatriaonline.indonaga.com/forum/app/index.php?for=event&do=changepass" : ((GameCanvas.IndexServer != mSystem.INDEX_SV_GLOBAL) ? "http://forum.knightageonline.com/app/index.php?for=event&do=resetpass&lang=store" : "http://forum.knightageonline.com/app/index.php?for=event&do=resetpass&lang=en"));
		...
		case 8:
		{
			...
			sbyte b = (GameCanvas.lowGraphic ? ((sbyte)1) : ((sbyte)0));
	...
	public static void loadVersionGame()
	{
		...
		if (array != null && !new DataInputStream(array).readUTF().Equals(GameMidlet.version))
		{
			Rms.deleteAllRecord();
		}
	}
	...
	public void checkLoginAgain(sbyte indexSv)
	{
		try
		{
			...
			if (b == indexSv && CRes.loadRMS("user_pass") != null)
			{
				try
				{
					loadUser_Pass();
