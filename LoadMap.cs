...
public class LoadMap
{
	...
	public void loadmap(sbyte[] mbyte)
	{
		try
		{
			...
			for (int i = 0; i < mapW * mapH; i++)
			{
				...
				else if (idTile != 12 && idTile != 13 && idTile != 14 && idTile != 15)
				{
					_ = idTile;
				}
				if (b >= fStand || b == 0)
	...
	public static string getTimeSpecialRegion()
	{
		...
		long num3 = num / 60;
		long num4 = num;
		string text = ((num2 > 0) ? (num2 + "h" + num % 3600 / 60 + "'") : ((num3 <= 0) ? (num4 + "s") : (num3 + "p" + num % 60 + "s")));
		return GameScreen.nameSpecialRegion + string.Empty + text;
	}
