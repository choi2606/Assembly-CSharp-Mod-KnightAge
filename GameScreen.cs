...
public class GameScreen : MainScreen
{
	...
	public void doSellItem()
	{
		...
		TabShopNew tabShopNew2 = new TabShopNew(new mVector("info"), MainTabNew.SELLITEM, T.gianHang, -1, 0);
	...
	public void paintMonsterEffect(mGraphics g)
	{
		...
		{
			((EffectMonster)EffectMonster.listEffectMonster.elementAt(i)).paint(g);
		}
	}
	...
	public void paintMonsterDieEffect(mGraphics g)
	{
		for (int i = 0; i < EffectMonster.listEffectMonster.size(); i++)
		{
			((EffectMonster)EffectMonster.listEffectMonster.elementAt(i)).paintDie(g);
		}
	}
	...
	public override void paint(mGraphics g)
	{
		...
		try
		{
			...
			{
				((ThacNuoc)LoadMap.Thacnuoc.elementAt(j)).paint(g);
			}
			for (int k = 0; k < vecStep.size(); k++)
			{
				((Point)vecStep.elementAt(k)).paint(g);
			}
			for (int l = 0; l < vecEffInMap.size(); l++)
			{
				((Point)vecEffInMap.elementAt(l)).paint(g);
			}
			...
			{
				Point point = (Point)LoadMap.vecPointChange.elementAt(num2);
				g.drawRegion(AvMain.imgSelect, 0, 0, 12, 16, LoadMap.mTranPointChangeMap[point.dis], point.x + GameCanvas.gameTick % 6 * point.vx, point.y + GameCanvas.gameTick % 6 * point.vy, 3, mGraphics.isFalse);
				AvMain.Font3dWhite(g, point.name, point.x2 + GameCanvas.gameTick % 6 * point.vx, point.y2 + GameCanvas.gameTick % 6 * point.vy, point.f);
			}
			...
			{
				((AnimateEffect)vecWeather.elementAt(num6)).paint(g);
			}
	...
	public void checkPaintNamearena()
	{
		...
		for (int j = 0; j < Vecplayers.size() - 1; j++)
		{
			...
			{
				MainObject mainObject2 = (MainObject)Vecplayers.elementAt(k);
	...
	public override void update()
	{
		try
		{
			...
			{
				((ThacNuoc)LoadMap.Thacnuoc.elementAt(m)).update();
			}
			if (help.Step >= 0)
			{
				help.updateHelp();
			...
			{
				IArrow obj = (IArrow)arrowsUp.elementAt(num2);
				obj.update();
				if (obj.wantDestroy)
				{
			...
			{
				((EffectMonster)EffectMonster.listEffectMonster.elementAt(num6)).update();
			}
		}
		catch (Exception ex)
		{
	...
	public void doMenuUseNgua(mVector vec)
	{
		...
		for (int i = 0; i < vec.size(); i++)
		{
			...
			if (mainItem != null)
			{
				iCommand iCommand2 = new iCommand(mainItem.itemName, 26, i, this);
				mVector3.addElement(iCommand2);
	...
	public static void addEffectKill(int type, int idFrom, sbyte temFrom, mVector vec)
	{
		...
		{
			if (idFrom != player.ID && EffectSkill.countSkillArena <= 3)
			{
	...
	public static void StartAddEffectSkill(int type, Object_Effect_Skill objfire, mVector vec)
	{
		...
		{
		case 119:
			addEffect2Vector(new EffectSkill(114, objfire, vec, 0));
			addEffect2Vector(new EffectSkill(115, objfire, vec, 0));
			return;
		case 118:
		{
			addEffect2Vector(new EffectSkill(65, objfire, vec, 0));
			...
			for (int k = 0; k < vec.size(); k++)
			{
				...
				if (object_Effect_Skill3 != null)
				{
					...
					addEffect2Vector(new EffectSkill(62, objfire, mVector5, 0));
		...
		case 117:
			addEffect2Vector(new EffectSkill(20, objfire, vec, 0));
			addEffect2Vector(new EffectSkill(91, objfire, vec, 0));
			return;
		case 116:
			addEffect2Vector(new EffectSkill(53, objfire, vec, 0));
			addEffect2Vector(new EffectSkill(77, objfire, vec, 0));
			return;
		case 123:
		{
			addEffect2Vector(new EffectSkill(60, objfire, vec, 0));
			...
			for (int j = 0; j < vec.size(); j++)
			{
				...
				if (object_Effect_Skill2 != null)
				{
					...
					addEffect2Vector(new EffectSkill(49, objfire, mVector4, 0));
		...
		case 122:
		{
			...
			for (int m = 0; m < vec.size(); m++)
			{
				...
				if (object_Effect_Skill5 != null)
				{
					...
					addEffect2Vector(new EffectSkill(51, objfire, mVector7, 0));
					mVector7.removeAllElements();
				}
			}
			addEffect2Vector(new EffectSkill(66, objfire, vec, 0));
			return;
		}
		case 121:
		{
			addEffect2Vector(new EffectSkill(34, objfire, vec, 0));
			...
			for (int l = 0; l < vec.size(); l++)
			{
				...
				if (object_Effect_Skill4 != null)
				{
					...
					addEffect2Vector(new EffectSkill(55, objfire, mVector6, 0));
		...
		case 120:
		{
			...
			for (int i = 0; i < vec.size(); i++)
			{
				...
				if (object_Effect_Skill != null)
				{
					...
					addEffect2Vector(new EffectSkill(54, objfire, mVector3, 0));
					mVector3.removeAllElements();
				}
			}
			addEffect2Vector(new EffectSkill(11, objfire, vec, 0));
			return;
		}
		}
		if (infoGame.isMapchienthanh() || infoGame.ismapHouse(GameCanvas.loadmap.idMap))
		{
			...
			{
				addEffect2Vector(new EffectSkill(type, objfire, vec, 0));
				return;
			}
	...
	public static void addEffectKill(int type, int idFrom, sbyte temFrom, int idTo, sbyte temTo, int hpshow, int hplast)
	{
		...
		addEffect2Vector(new EffectSkill(type, objkill, mVector3, 0));
	}
	...
	public static void addEffectKill(int type, int idFrom, sbyte temFrom, int idTo, sbyte temTo, int hpshow, int hplast, sbyte sub)
	{
		...
		mVector3.addElement(object_Effect_Skill);
		Object_Effect_Skill objkill = new Object_Effect_Skill((short)idFrom, temFrom);
		addEffect2Vector(new EffectSkill(type, objkill, mVector3, sub));
	}
	...
	public static void addEffectKill(int type, int idFrom, sbyte temFrom, mVector vec, sbyte sub)
	{
		...
		{
			addEffect2Vector(new EffectSkill(type, objkill, vec, sub));
			return;
		}
		for (int i = 0; i < vec.size(); i++)
		{
	...
	public static void StartEffect_Chiemthanh(int type, int idFrom, sbyte temFrom, mVector vec, sbyte sub)
	{
		...
		if (mainObject.x <= 216)
		{
			for (int i = 0; i < 3; i++)
			{
				EffectManager.addHiEffect(new EffectSkill(mainObject.x, mainObject.y + 12, mainObject.x + posSkill[0][i], mainObject.y + posSkill[1][i], 0));
			}
		}
		else if (mainObject.x >= 384)
		{
			for (int j = 0; j < 3; j++)
			{
				EffectManager.addHiEffect(new EffectSkill(mainObject.x, mainObject.y + 12, mainObject.x + posSkill[0][j], mainObject.y + posSkill[1][j], 1));
			}
		}
	}
	...
	{
		Object_Effect_Skill objkill = new Object_Effect_Skill((short)idFrom, temFrom);
		addEffect2Vector(new EffectSkill(type, objkill, vec, subType)
		{
