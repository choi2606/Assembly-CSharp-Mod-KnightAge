...
public class DataEffect
{
	...
	public void setDataType1(sbyte[] array)
	{
		...
		try
		{
			...
			int num3 = 0;
			int num4 = -1000000;
			int num5 = dataInputStream.readShort();
			for (int j = 0; j < num5; j++)
			{
				...
				for (int k = 0; k < b; k++)
				{
					...
					if (j == 0)
					{
						if (num4 < partFrame.dy + smallImage[partFrame.idSmallImg].h)
						{
							num4 = partFrame.dy + smallImage[partFrame.idSmallImg].h;
						}
			...
			if (dataInputStream.available() > 0)
			{
				...
				{
					FrameEff obj = (FrameEff)listFrame.elementAt(num12);
					obj.xShadow = dataInputStream.readByte();
					obj.yShadow = dataInputStream.readByte();
				}
			}
		}
		catch (Exception ex)
	...
	public void paintPet(mGraphics g, int idFrame, int x, int y, int way, mImage img)
	{
		...
		try
		{
			...
			for (int i = 0; i < frameEff.listPartTop.size(); i++)
			{
				...
				SmallImage obj = smallImage[partFrame.idSmallImg];
				int num = partFrame.dx;
				int num2 = obj.w;
				int num3 = obj.h;
				int num4 = obj.x;
