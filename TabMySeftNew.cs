public class TabMySeftNew : MainTabNew
{
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 0:
		{
			...
			Item item4 = (Item)Item.VecEquipPlayer.get(string.Empty + (idSelect + indexTab));
			if (item4 == null)
			{
				switch (num)
				{
				case -2:
					return;
				case -1:
					num = ((GameScreen.player.clazz == 2) ? 11 : ((GameScreen.player.clazz != 3) ? (8 + GameScreen.player.clazz) : 10));
					break;
				}
			}
			else
			{
				num = item4.type_Only_Item;
			}
			if (num == 14)
			{
				if (item4 != null)
				{
					GameCanvas.start_Pet_Info((PetItem)item4, MsgDialog.EQUIP);
				}
				return;
			}
			listItemMenu(num);
		...
		case 1:
		{
			Item item2 = (Item)vecItemMenu.elementAt(selectList);
			if (item2.setNameNull())
			{
				return;
			}
			GlobalService.gI().Use_Item((sbyte)item2.Id, idSelect);
			MainTabNew.timePaintInfo = 0;
			TabScreenNew.timeRepaint = 10;
			if (!GameCanvas.isTouch)
			{
		...
		case 3:
		{
			Item item3 = (Item)Item.VecEquipPlayer.get(string.Empty + idSelect);
			if (item3 != null && item3.ItemCatagory == 9)
			{
				GameCanvas.start_Pet_Info((PetItem)item3, MsgDialog.EQUIP);
			}
	...
	public void paintList(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			((Item)vecItemMenu.elementAt(i)).paintItem(g, xList + i * wsize + MainTabNew.wOneItem / 2 + ((num == 1) ? 1 : 0), yList + MainTabNew.wOneItem / 2, MainTabNew.wOneItem, 0, 0);
		}
		if (num > 0)
		{
			g.setColor(MainTabNew.color[2]);
	...
	public void paintInfoPlayer(mGraphics g, int x, int y, MainInfoItem[] mItemInfo, bool isNew)
	{
		...
		foreach (MainInfoItem mainInfoItem in mItemInfo)
		{
			...
			mFont obj = (isNew ? MainTabNew.setTextColor(Item.colorInfoItem[mainInfoItem.id]) : mFont.tahoma_7_black);
			string st = Item.nameInfoItem[mainInfoItem.id] + ": " + Item.getPercent(Item.isPercentInfoItem[mainInfoItem.id], mainInfoItem.value);
			obj.drawString(g, st, x, y, 0, mGraphics.isTrue);
			int num = 0;
