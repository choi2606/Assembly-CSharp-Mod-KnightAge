...
public class GameCanvas
{
	...
	public GameCanvas()
	{
		...
		try
		{
			...
			if (array != null)
			{
				if (new DataInputStream(array).readByte() == 1)
				{
					lowGraphic = true;
				}
				else
		...
		try
		{
			...
			if (array2 != null)
			{
				IndexRes = new DataInputStream(array2).readByte();
			}
		}
		catch (Exception)
		{
		...
		try
		{
			if (TemMidlet.DIVICE > 0)
			{
				...
				if (array3 != null)
				{
					IndexCharPar = new DataInputStream(array3).readShort();
				}
			}
			else
			{
		...
		try
		{
			if (TemMidlet.DIVICE > 0)
			{
				...
				if (array5 != null)
				{
					DataInputStream dataInputStream = new DataInputStream(array5);
