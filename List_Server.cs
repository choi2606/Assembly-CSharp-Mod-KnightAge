public class List_Server : MainScreen
{
	...
	{
		if (idSelect >= 0 && idSelect < vecListServer.size() && ((MainClan)vecListServer.elementAt(idSelect)).hang != -1)
		{
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 0:
		{
			MainObject mainObject6 = (MainObject)vecMyFriend.elementAt(idSelect);
			GlobalService.gI().Friend(3, mainObject6.name);
		...
		case 4:
		{
			...
			MainObject mainObject5 = (MainObject)vecListServer.elementAt(idSelect);
			if (mainObject5 != null && mainObject5.hang != -1)
			{
				if (typeList == 6)
				{
					GlobalService.gI().Re_Info_Other_Object(mainObject5.name, Info_Other_Player.THACH_DAU_INFO);
				}
				else
				{
					GlobalService.gI().Re_Info_Other_Object(mainObject5.name, Info_Other_Player.VIEW);
				}
			}
			break;
		}
		...
		case 9:
		{
			MainObject mainObject7 = (MainObject)vecListServer.elementAt(idSelect);
			if (mainObject7 != null)
			{
				GlobalService.gI().PhongCap_Clan(4, (sbyte)subIndex, mainObject7.name);
			}
	...
	public void setXmove()
	{
		...
		{
			st = ((MainObject)vecListServer.elementAt(idSelect)).infoObject;
		}
		if (typeList == 2)
		{
			num = 50;
