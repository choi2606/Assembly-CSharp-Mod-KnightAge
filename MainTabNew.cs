public class MainTabNew : AvMain
{
		...
		for (int num10 = 0; num10 <= num8; num10++)
		{
			for (int num11 = 0; num11 <= num6; num11++)
			{
				...
				if (num10 == num8)
				{
					if (num11 == num9)
					{
						g.drawImage(imgTab[num7], xlongwidth + longwidth - 32, ylongwidth + hSmall - 32, 0, mGraphics.isFalse);
					}
					else
					{
						g.drawImage(imgTab[num7], xlongwidth + longwidth - 32, ylongwidth + num11 * 32, 0, mGraphics.isFalse);
					}
				}
				else if (num11 == num9)
				{
					g.drawImage(imgTab[num7], xlongwidth + num10 * 32, ylongwidth + hSmall - 32, 0, mGraphics.isFalse);
				}
				else
				{
					g.drawImage(imgTab[num7], xlongwidth + num10 * 32, ylongwidth + num11 * 32, 0, mGraphics.isFalse);
				}
			}
		}
		for (int num12 = xlongwidth; num12 < xlongwidth + longwidth; num12 += 6)
		{
			g.fillRect(num12, ylongwidth + wOneItem, 4, 1, mGraphics.isFalse);
		}
	}
	...
	public void paintContentNew(mGraphics g, bool isOnlyName)
	{
		...
		int num2 = xlongwidth;
		int num3 = ylongwidth;
		g.setClip(num2 + 1, num3 + 1, longwidth - 2, hSmall - 2);
		if (!isOnlyName)
		{
			paintNameItem(g, num2 + longwidth / 2, num3 + wOneItem / 2 - 5, longwidth, name, colorName);
			if (listContent != null)
			{
				g.setClip(num2, num3 + wOneItem + 2, longwidth, hMaxContent - wOneItem - 2);
				g.translate(0, -listContent.cmx);
			}
			num3 += wOneItem - GameCanvas.hText + GameCanvas.hText / 4;
		}
		if (mPlusContent != null)
		{
			for (int i = 0; i < mPlusContent.Length; i++)
			{
				num3 += GameCanvas.hText;
				setTextColor(mPlusColor[i]).drawString(g, mPlusContent[i], num2 + num, num3 + 2, 0, mGraphics.isTrue);
			}
		}
		if (mPlusContent == null && moreInfoconten.size() > 0)
		{
		...
		for (int j = 0; j < moreInfoconten.size(); j++)
		{
			...
			if (mPlusColor != null)
			{
				Item.eff_UpLv.paintUpgradeEffect(num2 + num + mPlusContent[0].Length * 5 + 3 + 15 * j, num3 - GameCanvas.hText / 2 + ((mPlusContent.Length == 1) ? GameCanvas.hText : 0), 13, 13, g, 0);
			}
			else
			{
				Item.eff_UpLv.paintUpgradeEffect(num2 + num + 16 + 15 * j, num3 - GameCanvas.hText / 2 + GameCanvas.hText, 13, 13, g, 0);
			}
			if (infocontenNew.idimage == -1)
			{
				continue;
			...
			if (material != null)
			{
				if (mPlusContent != null && mPlusContent[0] != null)
				{
					material.paintItem_notnum(g, num2 + num + mPlusContent[0].Length * 5 + 3 + 15 * j + 1, num3 - GameCanvas.hText / 2 + 1 + ((mPlusContent.Length == 1) ? GameCanvas.hText : 0), 21, 1, 0);
				}
				else
				{
					material.paintItem_notnum(g, num2 + num + 16 + 15 * j, num3 - GameCanvas.hText / 2 + GameCanvas.hText + 1, 21, 1, 0);
				}
			}
			else
			{
		...
		if (mContent != null)
		{
			...
			{
				num3 += GameCanvas.hText;
				mFont.tahoma_7_white.drawString(g, T.level + pet.LvItem + " + " + pet.experience / 10 + "," + pet.experience % 10 + "%", num2 + num, num3 + 2, 0, mGraphics.isTrue);
				num3 += GameCanvas.hText;
				int num4 = pet.age / 24;
				int num5 = pet.age % 24;
				int num6 = (int)(pet.timeDefaultItemFashion - pet.getTimeItemFashion() / 60000);
				if (num6 > 0)
				{
					mFont.tahoma_7_red.drawString(g, T.sudungsau + " " + PaintInfoGameScreen.getStringTime(num6), num2 + num, num3 + 2, 0, mGraphics.isTrue);
					num3 += GameCanvas.hText;
				}
				mFont.tahoma_7_white.drawString(g, T.tuoi + num4 + "d " + num5 + "h", num2 + num, num3 + 2, 0, mGraphics.isTrue);
				num3 += GameCanvas.hText;
				if (pet.petAttack != null)
				{
					setTextColor(Item.colorInfoItem[pet.petAttack.id]).drawString(g, Item.nameInfoItem[pet.petAttack.id] + ": " + pet.petAttack.value + "-" + pet.petAttack.maxDam, num2 + num, num3 + 2, 0, mGraphics.isTrue);
				}
				num3 += GameCanvas.hText;
				mFont.tahoma_7_white.drawString(g, T.choan + ": " + pet.growpoint + "/" + pet.maxgrow, num2 + num, num3 + 2, 0, mGraphics.isTrue);
				num3 += GameCanvas.hText;
				for (int k = 0; k < T.mKynangPet.Length; k++)
				{
					mFont.tahoma_7_white.drawString(g, T.mKynangPet[k] + ": " + pet.mvaluetiemnang[k] + "/" + pet.maxtiemnang, num2 + num, num3 + 2, 0, mGraphics.isTrue);
					num3 += GameCanvas.hText;
				}
				for (int l = 0; l < pet.mInfo.Length; l++)
				{
					if (pet.mInfo[l].id > 6)
					{
						...
						setTextColor(Item.colorInfoItem[pet.mInfo[l].id]).drawString(g, st, num2 + num, num3 + 2, 0, mGraphics.isTrue);
						num3 += GameCanvas.hText;
					}
				}
			}
			else
			{
				for (int m = 0; m < mContent.Length; m++)
				{
					if (mContent[m] != null)
					{
						mFont mFont2 = null;
						mFont2 = ((mcolor == null) ? mFont.tahoma_7_white : setTextColor(mcolor[m]));
						mFont2.drawString(g, mContent[m], num2 + num, num3 + 2 + (m + 1) * GameCanvas.hText, 0, mGraphics.isTrue);
						if (mSubContent != null)
						{
							int num7 = mFont2.getWidth(mContent[m]) + 5;
							mFont2 = setTextColor(mSubColor[m]);
							mFont2.drawString(g, mSubContent[m], num2 + num7 + num, num3 + 2 + (m + 1) * GameCanvas.hText, 0, mGraphics.isTrue);
						}
					}
				}
			}
		...
		{
			mFont.tahoma_7b_white.drawString(g, name, num2 + longwidth / 2, num3 + wOneItem / 2 - 5, 2, mGraphics.isTrue);
		}
		GameCanvas.resetTrans(g);
	}
	...
	public static void paintNameItem(mGraphics g, int x, int y, int w, string name, int colorName)
	{
		...
		{
			setTextColorName(colorName).drawString(g, name, x, y, 2, mGraphics.isTrue);
			return;
		}
		...
		mFont obj = setTextColor(colorName);
		obj.drawString(g, namePaint[0], x, y - 6, 2, mGraphics.isTrue);
		obj.drawString(g, namePaint[1], x, y + 6, 2, mGraphics.isTrue);
	}
