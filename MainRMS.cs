...
public class MainRMS
{
	...
	public static void setLoadAuto(sbyte[] data)
	{
		...
		if (num && PaintInfoGameScreen.isShowInfoAuto)
		{
			if (flag)
			{
				showAuto += "\n";
		...
		int num2 = MsgDialog.MaxSkillBuff;
		if (data.Length - 7 >= num2)
		{
			num2 -= CRes.abs(num2 - (data.Length - 7)) + 1;
		}
	...
	public static void setSaveTouch()
	{
		...
		{
			PaintInfoGameScreen.isLevelPoint ? ((sbyte)1) : ((sbyte)0),
			PaintInfoGameScreen.isShowInfoAuto ? ((sbyte)1) : ((sbyte)0)
		};
		try
		{
