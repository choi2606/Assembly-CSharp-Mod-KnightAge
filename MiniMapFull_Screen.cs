	...
	public void init()
	{
		...
		if (GameCanvas.isTouch)
		{
			...
			if (imgtest == null || LoadMap.get_Item().idMap != idMap)
			{
				...
				{
					MiniMap.isAtMiniMap = false;
					MiniMap.CreateMiniMap(wMini, Main.isWindowsPhone ? ((sbyte)1) : ((sbyte)0));
	...
	public override void paint(mGraphics g)
	{
		...
		if (MiniMap.isLoadMiniMapOk && !MiniMap.isAtMiniMap)
		{
			if (Main.isWindowsPhone)
			{
				...
				{
					g.drawImage(imgData.img, 0, mGraphics.addYWhenOpenKeyBoard, 0, mGraphics.isFalse);
				}
			}
			else if (imgtest != null)
			{
				g.drawImage(imgtest, 0, mGraphics.addYWhenOpenKeyBoard, 0, mGraphics.isFalse);
			}
			int num = wMini;
