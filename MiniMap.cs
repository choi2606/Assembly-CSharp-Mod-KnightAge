...
public class MiniMap
{
	...
	public void setSize()
	{
		...
		isAtMiniMap = true;
		CreateMiniMap(wMini, Main.isWindowsPhone ? ((sbyte)1) : ((sbyte)0));
	}
	...
	public void paint(mGraphics g)
	{
		...
		g.setClip(0, mGraphics.addYWhenOpenKeyBoard, maxX * wMini, maxY * wMini);
		if (!isLoadMiniMapOk)
		{
			fWait++;
		...
		if (isLoadMiniMapOk && isAtMiniMap)
		{
			if (Main.isWindowsPhone)
			{
				...
				{
					g.drawImage(imgData.img, 0, mGraphics.addYWhenOpenKeyBoard, 0, mGraphics.isTrue);
				}
			}
			else if (imgMiniMap != null)
			{
				g.drawImage(imgMiniMap, 0, mGraphics.addYWhenOpenKeyBoard, 0, mGraphics.isTrue);
			}
			int num = wMini;
	...
	public static void SetTypeNPC(MainObject obj)
	{
		...
		{
			if (((MainQuest)MainQuest.vecQuestList.elementAt(i)).idNPC_From == obj.ID && (obj.typeNPC == 0 || obj.typeNPC == 2))
			{
				obj.typeNPC = 1;
			}
		}
		...
		{
			if (((MainQuest)MainQuest.vecQuestFinish.elementAt(j)).idNPC_To == obj.ID)
			{
				obj.typeNPC = 3;
			}
		}
		...
		{
			if (((MainQuest)MainQuest.vecQuestDoing_Main.elementAt(k)).idNPC_To == obj.ID && obj.typeNPC == 0)
			{
				obj.typeNPC = 2;
			}
		}
		...
		{
			if (((MainQuest)MainQuest.vecQuestDoing_Sub.elementAt(l)).idNPC_To == obj.ID && obj.typeNPC == 0)
			{
				obj.typeNPC = 2;
			}
		}
