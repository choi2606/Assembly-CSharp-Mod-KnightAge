...
public class MainObject : AvMain
{
	...
	public static DataSkillEff getEffMatNa(int id)
	{
		...
		if (dataSkillEff == null)
		{
			...
			GameData.getImgIcon((short)(id + GameData.ID_START_SKILL), dataSkillEff.typRequestImg);
	...
	public int getTypePart(int i)
	{
		...
		{
		case 0:
			if (idPartFashion[1] != -1)
			{
				return idPartFashion[1];
			}
			return leg;
		case 1:
			if (idPartFashion[0] != -1)
			{
				return idPartFashion[0];
			}
			return body;
		case 2:
			if (idPartFashion.Length > 6)
			{
				if (idPartFashion[6] != -1)
				{
					return idPartFashion[6];
				}
				return head;
			}
			return head;
		case 3:
			...
			{
				return idPartFashion[2];
			}
			return hat;
		case 4:
			return eye;
		case 5:
			if (idPartFashion[5] != -1)
			{
				return idPartFashion[5];
			}
			return hair;
		case 6:
			if (idPartFashion[4] != -1)
			{
				return idPartFashion[4];
			}
			return wing;
		default:
			return -1;
	...
	public static void updatefocus()
	{
		...
		if (GameScreen.ObjFocus.maxHp > 0 && (!PaintInfoGameScreen.isLevelPoint || Main.isPC) && GameScreen.ObjFocus.typeObject != 10)
		{
			...
			long num2 = 10L;
			framefocus = (sbyte)(num * num2 / GameScreen.ObjFocus.maxHp);
			if (framefocus > 9)
			{
				framefocus = 9;
			}
	...
	public static void resetDirection(MainObject idFrom, MainObject idTo)
	{
		...
		int direction = ((CRes.abs(num) <= CRes.abs(num2)) ? ((num2 > 0) ? 1 : 0) : ((num <= 0) ? 3 : 2));
	...
	public mVector vectorObjectNear()
	{
		...
		{
			MainObject mainObject = (MainObject)GameScreen.Vecplayers.elementAt(i);
			if (mainObject != null && mainObject != GameScreen.player && mainObject != this && mainObject.typeObject == 0 && getDistance(x, y, mainObject.x, mainObject.y - mainObject.hOne / 2) <= num)
			{
	...
	public bool isHairNotHat()
	{
		...
		for (int i = 0; i < num; i++)
		{
			if (Player.ID_HAIR_NO_HAT[i] == hair)
			{
				return true;
			}
		}
