public class NPCserver : MainObject
{
	...
	public override void paintName(mGraphics g, int id)
	{
		...
		MainTabNew.setTextColor(id).drawString(g, nameGiaotiep, p.x, p.y - 70, 2, mGraphics.isFalse);
	}
	...
	public bool checkNV()
	{
		...
		{
			if (((MainQuest)MainQuest.vecQuestList.elementAt(i)).idNPC_From == ID)
			{
				return true;
			}
		}
		...
		{
			if (((MainQuest)MainQuest.vecQuestFinish.elementAt(j)).idNPC_To == ID)
			{
				return true;
			}
		}
		...
		{
			if (((MainQuest)MainQuest.vecQuestDoing_Main.elementAt(k)).idNPC_To == ID)
			{
				return true;
			}
		}
		...
		{
			if (((MainQuest)MainQuest.vecQuestDoing_Sub.elementAt(l)).idNPC_To == ID)
			{
				return true;
			}
		}
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 0:
			...
			{
				((MainQuest)MainQuest.vecQuestList.elementAt(subIndex)).beginQuest();
			}
			break;
		case 1:
			if (subIndex >= 0 && subIndex <= MainQuest.vecQuestFinish.size())
			{
				((MainQuest)MainQuest.vecQuestFinish.elementAt(subIndex)).beginQuest();
			}
			break;
		case 2:
			if (subIndex >= 0 && subIndex <= MainQuest.vecQuestDoing_Main.size())
			{
				((MainQuest)MainQuest.vecQuestDoing_Main.elementAt(subIndex)).show_Info_Quest_Doing();
			}
			break;
		case 3:
			if (subIndex >= 0 && subIndex <= MainQuest.vecQuestDoing_Sub.size())
			{
				((MainQuest)MainQuest.vecQuestDoing_Sub.elementAt(subIndex)).show_Info_Quest_Doing();
			}
			break;
		case 4:
