public class MsgChat : MainDialog
{
	...
	public override void paint(mGraphics g)
	{
		...
		if (vecChatTab.size() > 0 && curentfocus != null)
		{
			...
			mFont.tahoma_7b_white.drawString(g, empty, xselect + wMainTab / 2, num2 + wOneItem / 2 - 7, 2, mGraphics.isTrue);
			num2 += wOneItem;
	...
	public override void update()
	{
		...
		if (vecChatTab.size() > 0 && curentfocus != null)
		{
			...
			{
				if ((i < min || i >= max) && ((ChatDetail)vecChatTab.elementAt(i)).isNew)
				{
					if (i < min)
					{
						isLeft = true;
	...
	public static void setIndexFocus(string name)
	{
		for (int i = 0; i < vecChatTab.size(); i++)
		{
			if (((ChatDetail)vecChatTab.elementAt(i)).name.CompareTo(name) == 0)
			{
				idSelect = i;
				break;
			}
