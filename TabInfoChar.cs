	...
	public override void updatePointer()
	{
		if (GameCanvas.isPointerSelect)
		{
			for (int i = 0; i < 4; i++)
			{
				...
				if (GameCanvas.isPoint(y: yBegin + GameCanvas.hText + 4 + hitem * i + plusTouch + 2 - 5 - plusTouch / 2, x: num - 2 - plusTouch / 2, w: 32 + plusTouch, h: 21 + plusTouch))
				{
					idSelect = i;
					cmdSetPoint.perform();
