...
public class MsgDialog : MainDialog
{
	...
	public override void commandTab(int index, int subIndex)
	{
		...
		case 5:
			...
			MainItem.setAddHotKey(1, isUutien != 0);
			MainItem.setAddHotKey(0, isUutien != 0);
			MainRMS.setSaveAuto();
	...
	public override void paint(mGraphics g)
	{
		...
		case 1:
		{
			...
			else
			{
				...
				{
					g.drawRegion(MainTabNew.imgTab[2], 0, 0, 32, 14, 0, GameCanvas.hw - 32 + num20 * 32, yDia + 11, 0, mGraphics.isTrue);
				}
			}
			...
			for (int num21 = 0; num21 < strinfo.Length; num21++)
			{
				mFont.tahoma_7_black.drawString(g, strinfo[num21], xDia + 11, yDia + 27 + (num21 + 1) * GameCanvas.hText, 0, mGraphics.isTrue);
			}
			break;
		}
		case 2:
		{
			...
			int num25 = yDia + GameCanvas.hCommand + 3;
			int num26 = 2;
			if (Player.party != null)
			{
				...
				{
					g.setColor(11904141);
					g.fillRect(xDia + 9, num25 - 2 + idSelect * hItem, wDia - 17, hItem - 1, mGraphics.isTrue);
				}
				if (Player.party != null)
				{
					for (int num27 = 0; num27 < Player.party.vecPartys.size(); num27++)
					{
						...
						{
							AvMain.Font3dColorAndColor(g, objectParty.name + " " + T.Lv + objectParty.Lv, xDia + 11, num25, 0, 7, 0);
						}
						else
						{
							mFont.tahoma_7b_white.drawString(g, objectParty.name + " " + T.Lv + objectParty.Lv, xDia + 11, num25, 0, mGraphics.isTrue);
						}
						...
						g.setColor(0);
						g.fillRect(xDia + 11, num25 + 14 - num26, 42, 4, mGraphics.isTrue);
						g.setColor(8062494);
						g.fillRect(xDia + 12, num25 + 15 - num26, 40, 2, mGraphics.isTrue);
						g.setColor(16197705);
						g.fillRect(xDia + 12, num25 + 15 - num26, 40 * objectParty.hp / objectParty.maxhp, 2, mGraphics.isTrue);
						string text = "map";
						...
						mFont.tahoma_7_white.drawString(g, text + " - " + T.Area + (objectParty.idArea + 1), xDia + 11, num25 + 20 - num26 * 2, 0, mGraphics.isTrue);
						num25 += hItem;
						if (num27 < Player.party.vecPartys.size() - 1)
						{
							g.setColor(AvMain.color[4]);
							g.fillRect(xDia + 12, num25 - 3, wDia - 24, 1, mGraphics.isTrue);
						}
					}
				}
			}
			...
			{
				mFont.tahoma_7_black.drawString(g, T.noParty, xDia + wDia / 2, num25, 2, mGraphics.isTrue);
			}
			base.paint(g);
			break;
		}
		case 4:
		{
			...
			int num22 = yDia + hItem + 11;
			int num23 = xDia + 30 - (GameCanvas.isTouch ? 10 : 0);
			g.setColor(MainTabNew.color[0]);
			for (int num24 = 0; num24 < T.mAuto.Length; num24++)
			{
				mFont.tahoma_7_white.drawString(g, T.mAuto[num24], num23, num22, 0, mGraphics.isTrue);
				int width3 = mFont.tahoma_7_black.getWidth(T.mAuto[0]);
				g.fillRect(num23 + width3, num22 - 3, 35, 18, mGraphics.isTrue);
				mFont.tahoma_7_white.drawString(g, "   " + mHPMP[num24] + "     %", num23 + 3 + width3, num22, 0, mGraphics.isTrue);
				num22 += hItem;
			}
			mFont.tahoma_7_white.drawString(g, T.mUtien[isUutien], num23, num22, 0, mGraphics.isTrue);
			if (!GameCanvas.isTouch)
			{
				g.drawRegion(AvMain.imgSelect, 0, 0, 12, 16, 4, num23 - 4 + GameCanvas.gameTick % 3, yDia + hItem + 17 + idSelect * hItem, mGraphics.VCENTER | mGraphics.RIGHT, mGraphics.isTrue);
			}
			GameCanvas.resetTrans(g);
			paintCmd(g);
			break;
		...
		{
			paintFormList(g, xDia, yDia, wDia, hDia, T.auto);
			int num10 = yDia + hItem + 11;
			int num11 = xDia + 30 - (GameCanvas.isTouch ? 10 : 0);
			g.setColor(MainTabNew.color[3]);
			for (int n = 0; n < T.mAutoItem.Length; n++)
			{
				mFont.tahoma_7b_white.drawString(g, T.mAutoItem[n], num11, num10, 0, mGraphics.isTrue);
				int width = mFont.tahoma_7b_white.getWidth(T.mAutoItem[n]);
				mFont.tahoma_7_white.drawString(g, "< " + T.mValueAutoItem[n][mvalueItem[n]] + " >", num11 + 3 + width, num10, 0, mGraphics.isTrue);
				num10 += hItem;
			}
			if (!GameCanvas.isTouch)
			{
				g.drawRegion(AvMain.imgSelect, 0, 0, 12, 16, 4, num11 - 4 + GameCanvas.gameTick % 3, yDia + hItem + 17 + idSelect * hItem, mGraphics.VCENTER | mGraphics.RIGHT, mGraphics.isTrue);
			}
			GameCanvas.resetTrans(g);
			paintCmd(g);
			break;
		...
		case 8:
		{
			...
			{
				((Skill)TabSkillsNew.vecPaintSkill.elementAt(Autobuff[num30][2])).paint(g, num29, num28, 3);
		...
		case 10:
		{
			...
			if (StepShow < 2)
			{
				for (int num15 = 0; num15 < posItemNguyenlieu.Length; num15++)
				{
					g.drawImage(AvMain.imgHotKey, posItemNguyenlieu[num15][0], posItemNguyenlieu[num15][1], 3, mGraphics.isTrue);
				}
				if (StepShow == 0)
				{
					for (int num16 = 0; num16 < datanguyenlieu.Length; num16++)
					{
						if (timeShow >= 4 && datanguyenlieu[num16] != null)
						{
							datanguyenlieu[num16].paintItem(g, posItemNguyenlieu[num16][0], posItemNguyenlieu[num16][1], 21, 1, 0);
						}
					}
				}
			}
			...
			{
				for (int num17 = 0; num17 < strinfo.Length; num17++)
				{
					mFont.tahoma_7_white.drawString(g, strinfo[num17], xDia + 8, yDia + GameCanvas.hCommand + 2 + num17 * GameCanvas.hText, 0, mGraphics.isTrue);
				}
				int num18 = posItemNguyenlieu.Length - 1;
				if (sanpham != null)
				{
					sanpham.paintItem(g, posItemNguyenlieu[num18][0], posItemNguyenlieu[num18][1], 21, 1, 0);
				}
				g.drawImage(AvMain.imgHotKey, posItemNguyenlieu[num18][0], posItemNguyenlieu[num18][1], 3, mGraphics.isTrue);
			}
			for (int num19 = 0; num19 < TabRebuildItem.vecEffRe.size(); num19++)
			{
				((MainEffect)TabRebuildItem.vecEffRe.elementAt(num19)).paint(g);
			}
			break;
		}
		case 11:
		{
			...
			{
				((MainEffect)TabRebuildItem.vecEffRe.elementAt(num35)).paint(g);
			}
			break;
		}
		case 12:
		{
			...
			int num6 = hUpdate - 10;
			int num7 = yDia + hDia - num6 - 25 - 5;
			g.setColor(2698542);
			g.fillRect(num5 - 4, num7 + 15, wUpdate + 2, num6, mGraphics.isTrue);
			g.fillRect(num5 - 4 + 1, num7 + 14, wUpdate, 1, mGraphics.isTrue);
			g.fillRect(num5 - 4 + 1, num7 + 15 + num6, wUpdate, 1, mGraphics.isTrue);
			g.setColor(3027507);
			g.fillRect(num5 - 4 + 1, num7 + 15, wUpdate, num6, mGraphics.isTrue);
			int num8 = 0;
			if (maxupdate > 0 && curupdate > 0)
			{
				num8 = curupdate * wUpdate / maxupdate;
				if (num8 <= 0)
				{
					num8 = 1;
				}
				else if (num8 > wUpdate)
				{
					num8 = wUpdate;
				}
				g.setColor(10339648);
				g.fillRect(num5 - 4 + 1, num7 + 15, num8, num6, mGraphics.isTrue);
			}
			int num9 = curupdate * 100 / maxupdate;
			mFont.tahoma_7b_white.drawString(g, num9 + "%", xDia + wDia / 2, num7, 2, mGraphics.isTrue);
			break;
		}
		case 13:
		{
			paintFormList(g, xDia, yDia, wDia, hDia, T.SetMusic);
			int num12 = yDia + hItem + 11;
			int num13 = xDia + 30 - (GameCanvas.isTouch ? 10 : 0);
			g.setColor(MainTabNew.color[3]);
			for (int num14 = 0; num14 < T.mVolume.Length; num14++)
			{
				mFont.tahoma_7b_white.drawString(g, T.mVolume[num14], num13, num12, 0, mGraphics.isTrue);
				int width2 = mFont.tahoma_7b_white.getWidth(T.mVolume[num14]);
				mFont.tahoma_7_white.drawString(g, "< " + ((mvalueVolume[num14] != 0) ? T.off : T.on) + " >", num13 + 3 + width2, num12, 0, mGraphics.isTrue);
				num12 += hItem;
			}
			if (!GameCanvas.isTouch)
			{
				g.drawRegion(AvMain.imgSelect, 0, 0, 12, 16, 4, num13 - 4 + GameCanvas.gameTick % 3, yDia + hItem + 17 + idSelect * hItem, mGraphics.VCENTER | mGraphics.RIGHT, mGraphics.isTrue);
			}
			GameCanvas.resetTrans(g);
			paintCmd(g);
			break;
	...
	public override void updatekey()
	{
		...
		if (cmdList != null)
		{
			...
			if (!GameCanvas.isTouch && num > 0)
			{
				...
				if (typeQuest == 2 && (iCommand)cmdList.elementAt(idCommand) == cmdClose)
				{
					idCommand = 0;
				}
				if (num2 != idCommand)
	...
	public override void updatePointer()
	{
		...
		if (cmdList != null)
		{
			...
			{
				((iCommand)cmdList.elementAt(j)).updatePointer();
			}
		}
	}
