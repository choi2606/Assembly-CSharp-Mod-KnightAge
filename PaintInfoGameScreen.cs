public class PaintInfoGameScreen
{
	...
	public void paintBuffPlayer(mGraphics g)
	{
		...
		{
			_ = (MainBuff)GameScreen.player.vecBuff.elementAt(i);
		}
	}
	...
	private void paintChangeSkill(mGraphics g)
	{
		for (int i = 0; i < 10; i++)
		{
			...
			if (hotKey.type == HotKey.SKILL)
			{
				MainListSkill.getSkillFormId(hotKey.id).paint(g, xPaintSkill + i % 5 * wSkill + 11, num + yPaintSkill + 11, 3);
			}
	...
	public void paintInfoThachDau(mGraphics g, int x, int y)
	{
		...
		if (idCharLoiDai1 != -1)
		{
			...
			if (mainObject.hp > 0)
			{
				...
				long num6 = num;
				int num7 = (int)(num5 * num6 / num4);
				int num8 = num + 1;
				g.setClip(x, y + 4, num8 - (num - num7), 7);
			...
			{
				long num11 = timeThachdau - mSystem.currentTimeMillis() / 1000;
				g.drawRegion(AvMain.imgBackInfo, 0, 0, 140, 20, 0, GameCanvas.w / 2, y + 35, 3, mGraphics.isFalse);
				mFont.tahoma_7_white.drawString(g, T.TimeThachDau + num11, GameCanvas.w / 2, y + 30, 2, mGraphics.isFalse);
			}
			return;
		}
		int num12 = GameCanvas.w / 2 - 20;
		int num13 = num12 / (WBlackclolor / 2);
		if (num13 < 0)
		{
			num13 = 1;
		}
		...
		else
		{
			...
			g.drawRegion(AvMain.imgcolorhpmp_back, 2, 0, WBlackclolor - 2, 9, 0, x + num12 - (WBlackclolor - 2) + 2, y + 3, 0, mGraphics.isTrue);
			g.drawRegion(AvMain.imgcolorhpmp_back, 2, 9, WBlackclolor - 2, 9, 0, x + num12 - (WBlackclolor - 2) + 2, y + 15, 0, mGraphics.isTrue);
			int num14 = (num12 - (WBlackclolor - 2) * 2) / 5;
			if (num14 <= 0)
			{
				num14 = 1;
			}
		...
		{
			long num15 = GameScreen.player.maxHp;
			long num16 = GameScreen.player.hp;
			long num17 = num12;
			int num18 = (int)(num16 * num17 / num15);
			int num19 = num12 + 1;
			g.setClip(x, y + 4, num19 - (num12 - num18), 7);
			int num20 = num12 / (WRedclor / 2);
			if (num20 < 0)
			{
				num20 = 1;
			}
			...
			{
				g.drawRegion(AvMain.imgcolorhpmp, 0, 0, WRedclor - 2, 7, 0, x + 1, y + 4, 0, mGraphics.isTrue);
				g.drawRegion(AvMain.imgcolorhpmp, 2, 0, WRedclor - 2, 7, 0, x + 1 + num12 - (WRedclor - 2), y + 4, 0, mGraphics.isTrue);
				int num21 = (num12 - (WRedclor - 2) * 2) / 5;
				if (num21 <= 0)
				{
					num21 = 1;
				}
				for (int l = 0; l < num21 + 1; l++)
				{
		...
		{
			long num22 = GameScreen.player.maxMp;
			long num23 = GameScreen.player.mp;
			long num24 = num12;
			long num25 = num23 * num24;
			int num26 = num12 + 2;
			int num27 = (int)(num25 / num22);
			g.setClip(x, y + 16, num26 - (num12 - num27), 7);
			int num28 = num12 / (WRedclor / 2);
			if (num28 < 0)
			{
				num28 = 1;
			}
			...
			else
			{
				...
				g.drawRegion(AvMain.imgcolorhpmp, 2, 7, WRedclor - 2, 7, 0, x + 1 + num12 - (WRedclor - 2), y + 16, 0, mGraphics.isTrue);
				int num29 = (num12 - (WRedclor - 2) * 2) / 5;
				if (num29 <= 0)
				{
					num29 = 1;
				}
				for (int m = 0; m < num29 + 1; m++)
				{
		...
		{
			mFont.tahoma_7_white.drawString(g, GameScreen.player.hp + "/" + GameScreen.player.maxHp, num12 / 2, y + 2, 2, mGraphics.isTrue);
		}
		else
		{
			...
			mFont3.drawString(g, GameScreen.player.hp + "/" + GameScreen.player.maxHp, num12 / 2, y + 2, 2, mGraphics.isTrue);
		}
		if (!GameScreen.player.overMP)
		{
			mFont.tahoma_7_white.drawString(g, GameScreen.player.mp + "/" + GameScreen.player.maxMp, num12 / 2, y + 14, 2, mGraphics.isFalse);
		}
		else
		{
			...
			mFont4.drawString(g, GameScreen.player.mp + "/" + GameScreen.player.maxMp, num12 / 2, y + 14, 2, mGraphics.isTrue);
		}
		if (timeThachdau - mSystem.currentTimeMillis() / 1000 > 0)
		{
			long num30 = timeThachdau - mSystem.currentTimeMillis() / 1000;
			g.drawRegion(AvMain.imgBackInfo, 0, 0, 140, 20, 0, GameCanvas.w / 2, y + 35, 3, mGraphics.isFalse);
			mFont.tahoma_7_white.drawString(g, T.TimeThachDau + num30, GameCanvas.w / 2, y + 30, 2, mGraphics.isFalse);
		}
	}
	...
	public void paintInfoThachDauOtherPlayer(mGraphics g, int x, int y)
	{
		...
		if (idCharLoiDai2 != -1)
		{
			...
			if (mainObject.hp > 0)
			{
				...
				long num6 = num;
				int num7 = (int)(num5 * num6 / num4);
				g.setClip(x + (num - num7), y + 4, GameCanvas.w / 2 - (num - num7), 7);
				int num8 = num / (WRedclor / 2);
				if (num8 < 0)
				{
					num8 = 1;
				}
		...
		else
		{
			...
			GameCanvas.resetTrans(g);
			int num10 = GameCanvas.w / 2 - 20;
			int num11 = num10 / (WBlackclolor / 2);
			if (num11 < 0)
			{
				num11 = 1;
			}
			...
			else
			{
				...
				int num12 = (num10 - (WBlackclolor - 2) * 2) / 5;
				if (num12 <= 0)
				{
					num12 = 1;
				}
				for (int k = 0; k < num12 + 1; k++)
				{
					g.drawRegion(AvMain.imgcolorhpmp_back, 10, 0, 15, 9, 0, x + WBlackclolor - 2 + k * 5, y + 3, 0, mGraphics.isTrue);
					g.drawRegion(AvMain.imgcolorhpmp_back, 10, 9, 15, 9, 0, x + WBlackclolor - 2 + k * 5, y + 15, 0, mGraphics.isTrue);
				}
			...
			{
				long num13 = GameScreen.ObjFocus.maxHp;
				long num14 = GameScreen.ObjFocus.hp;
				long num15 = num10;
				int num16 = (int)(num14 * num15 / num13);
				g.setClip(x + (num10 - num16), y + 4, GameCanvas.w / 2 - (num10 - num16), 7);
				int num17 = num10 / (WRedclor / 2);
				if (num17 < 0)
				{
					num17 = 1;
				}
				...
				else
				{
					g.drawRegion(AvMain.imgcolorhpmp, 0, 0, WRedclor - 2, 7, 0, x + 1, y + 4, 0, mGraphics.isTrue);
					g.drawRegion(AvMain.imgcolorhpmp, 2, 0, WRedclor - 2, 7, 0, GameCanvas.w - WBlackclolor, y + 4, 0, mGraphics.isTrue);
					int num18 = (num10 - (WRedclor - 2) * 2) / 5;
					if (num18 <= 0)
					{
						num18 = 1;
					}
					for (int l = 0; l < num18 + 1; l++)
					{
						g.drawRegion(AvMain.imgcolorhpmp, 10, 0, 15, 7, 0, x + WRedclor - 2 + l * 5, y + 4, 0, mGraphics.isTrue);
					}
				}
			...
			{
				long num19 = GameScreen.ObjFocus.maxMp;
				long num20 = GameScreen.ObjFocus.mp;
				long num21 = num10;
				int num22 = (int)(num20 * num21 / num19);
				g.setClip(x + (num10 - num22), y + 16, GameCanvas.w / 2 - (num10 - num22), 7);
				int num23 = num10 / (WRedclor / 2);
				if (num23 < 0)
				{
					num23 = 1;
				}
				...
				else
				{
					...
					int num24 = (num10 - (WRedclor - 2) * 2) / 5;
					if (num24 <= 0)
					{
						num24 = 1;
					}
					for (int m = 0; m < num24 + 1; m++)
					{
						g.drawRegion(AvMain.imgcolorhpmp, 10, 7, 15, 7, 0, x + WRedclor - 2 + m * 5, y + 16, 0, mGraphics.isTrue);
					}
				}
			...
			g.endClip();
			GameCanvas.resetTrans(g);
			AvMain.Font3dColor(g, GameScreen.ObjFocus.name.ToUpper() + " Lv: " + GameScreen.ObjFocus.Lv, x + num10 - 2, y + 24, 1, 0);
			if (!GameScreen.ObjFocus.overHP)
			{
				mFont.tahoma_7_white.drawString(g, GameScreen.ObjFocus.hp + "/" + GameScreen.ObjFocus.maxHp, x + num10 / 2, y + 2, 2, mGraphics.isTrue);
			}
			else
			{
				mFont mFont3 = mFont.tahoma_7_white;
				...
				mFont3.drawString(g, GameScreen.ObjFocus.hp + "/" + GameScreen.ObjFocus.maxHp, x + num10 / 2, y + 2, 2, mGraphics.isTrue);
			}
			if (!GameScreen.ObjFocus.overMP)
			{
				mFont.tahoma_7_white.drawString(g, GameScreen.ObjFocus.mp + "/" + GameScreen.ObjFocus.maxMp, x + num10 / 2, y + 14, 2, mGraphics.isFalse);
				return;
			}
			mFont mFont4 = mFont.tahoma_7_white;
			if (MainObject.countmp > 5)
			...
			mFont4.drawString(g, GameScreen.ObjFocus.mp + "/" + GameScreen.ObjFocus.maxMp, x + num10 / 2, y + 14, 2, mGraphics.isTrue);
		}
	}
	...
	public void paintInfoFocus(mGraphics g)
	{
		...
		if (objFocus.maxHp > 0 && objFocus.hp > 0)
		{
			...
			num4 = (int)(num5 * num6 / objFocus.maxHp);
			if (num4 <= 0)
			{
				num4 = 1;
			}
	...
	public void updatePoiterAll()
	{
		...
		if (GameCanvas.isPointerSelect)
		{
			...
			if (GameCanvas.isPointerSelect)
			{
				...
				if (GameCanvas.isPoint(140, 0, 107, 22))
				{
					GameCanvas.isPointerSelect = false;
					selectPointer(5);
				}
	...
	public void paintChangeTab(mGraphics g)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			if (hotKey != null && hotKey.type == HotKey.SKILL)
			{
				MainListSkill.getSkillFormId(hotKey.id).paint(g, x, y, 3);
			}
			else if (hotKey != null && hotKey.type == HotKey.POTION)
			{
	...
	{
		if (GameCanvas.loadmap.idMap != 83 && GameCanvas.loadmap.idMap != 84 && GameCanvas.loadmap.idMap != 85 && GameCanvas.loadmap.idMap != 86)
		{
			return GameCanvas.loadmap.idMap == 87;
		}
		return true;
	}
	...
	{
		if (!isMapChienTruong(idMap))
		{
			return isMapCountTime(idMap);
		}
		return true;
	}
