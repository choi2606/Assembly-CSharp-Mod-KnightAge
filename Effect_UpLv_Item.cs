public class Effect_UpLv_Item
{
	...
	public void paintUpgradeEffect(int x, int y, int upgrade, int w, mGraphics g, int lech)
	{
		...
		{
			int num3 = x - w / 2 + upgradeEffectX(GameCanvas.gameTick - i * wnew, w);
			int num4 = y - w / 2 + upgradeEffectY(GameCanvas.gameTick - i * wnew, w);
		...
		if (upgrade >= 6 && upgrade < 11)
		{
			...
			{
				int num5 = x - w / 2 + upgradeEffectX(GameCanvas.gameTick - w * 2 - j * wnew, w);
				int num6 = y - w / 2 + upgradeEffectY(GameCanvas.gameTick - w * 2 - j * wnew, w);
		...
		if (upgrade >= 11)
		{
			...
			{
				int num7 = x - w / 2 + upgradeEffectX(GameCanvas.gameTick - w * 13 / 10 - k * wnew, w);
				int num8 = y - w / 2 + upgradeEffectY(GameCanvas.gameTick - w * 13 / 10 - k * wnew, w);
			...
			{
				int num9 = x - w / 2 + upgradeEffectX(GameCanvas.gameTick - w * 13 / 5 - l * wnew, w);
				int num10 = y - w / 2 + upgradeEffectY(GameCanvas.gameTick - w * 13 / 5 - l * wnew, w);
