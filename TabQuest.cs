public class TabQuest : MainTabNew
{
	...
	public void paintInfoQuest(mGraphics g)
	{
		...
		g.setClip(num2 + 1, num3 + 1, MainTabNew.longwidth - 2, hSmall - 2);
		MainTabNew.paintNameItem(g, num2 + MainTabNew.longwidth / 2, num3 + MainTabNew.wOneItem / 2 - 5, MainTabNew.longwidth, name, colorName);
		num3 += MainTabNew.wOneItem - GameCanvas.hText + GameCanvas.hText / 4;
		GameCanvas.resetTrans(g);
		scr.setStyle(mContent.Length + 2, GameCanvas.hText + 2, num2, num3 + MainTabNew.wOneItem + 2 - GameCanvas.hText * 2, MainTabNew.longwidth, MainTabNew.hMaxContent + GameCanvas.hText, styleUPDOWN: true, 1);
		scr.setClip(g, num2, num3 + MainTabNew.wOneItem + 2 - GameCanvas.hText, MainTabNew.longwidth, MainTabNew.hMaxContent - MainTabNew.wOneItem - 2 - iCommand.hButtonCmd / 2);
		if (mContent != null)
		{
			for (int i = 0; i < mContent.Length; i++)
			{
				if (mContent[i] != null)
				{
					...
					mFont2.drawString(g, mContent[i], num2 + num, num3 + 2 + (i + 1) * GameCanvas.hText, 0, mGraphics.isTrue);
					if (mSubContent != null)
					{
						int num4 = mFont2.getWidth(mContent[i]) + 5;
						mFont2 = MainTabNew.setTextColor(mSubColor[i]);
						mFont2.drawString(g, mSubContent[i], num2 + num4 + num, num3 + 2 + (i + 1) * GameCanvas.hText, 0, mGraphics.isTrue);
					}
				}
			}
		}
